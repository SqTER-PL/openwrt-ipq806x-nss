							...
							ip_addr_t ip_src_addr, ip_addr_t ip_dest_addr)
{
	...
	if (unlikely(!ci)) {
		...
		src_ni = ecm_nss_ipv6_node_establish_and_ref(feci, efeici.from_dev, efeici.from_mac_lookup_ip_addr, from_list, from_list_first, src_node_addr, skb);
		...
		dest_ni = ecm_nss_ipv6_node_establish_and_ref(feci, efeici.to_dev, efeici.to_mac_lookup_ip_addr, to_list, to_list_first, dest_node_addr, skb);
