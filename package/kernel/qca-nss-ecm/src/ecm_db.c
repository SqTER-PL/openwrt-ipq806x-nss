...
#elseif (LINUX_VERSION_CODE < KERNEL_VERSION(4, 13, 0))
	nf_ct_iterate_cleanup(&init_net, ecm_db_iproute_connection_cmp_and_kill, 0, 0, 0);
#else
	nf_ct_iterate_cleanup_net(&init_net, ecm_db_iproute_connection_cmp_and_kill, 0, 0, 0);
#endif
	return NOTIFY_DONE;
}
...
#else
	nf_ct_iterate_cleanup_net(&init_net, ecm_db_ip6route_connection_cmp_and_kill, 0, 0, 0);
#endif
	kfree(work);
